...
export function UsageCharts({ data }: UsageChartsProps) {
  ...
  // 為餅圖準備數據
  const tokenPieData = hasModelData
  ...
  const costPieData = hasModelData
    ? Object.entries(data.models).map(([name, model]) => ({
        name,
  ...
  // 為對比圖準備模型比較數據
  const modelComparisonData = hasModelData
    ? Object.entries(data.models).map(([name, model]) => ({
        name:
          name === 'openai'
            ? 'OpenAI'
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground mt-1">
              ...
              {data.total.total_requests > 0
                ? Math.round(
                    data.total.total_tokens / data.total.total_requests
                  )
                : '-'}{' '}
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {data.total.total_cost > 0
                ? `$${data.total.total_cost.toLocaleString(undefined, {
            ...
            <div className="text-xs text-muted-foreground mt-1">
              ...
              {data.total.total_tokens > 0
                ? `$${(
                    (data.total.total_cost / data.total.total_tokens) *
                    1000
                  ).toFixed(3)}`
                : '-'}
            </div>
        ...
        <Card className="border border-gray-100 dark:border-gray-800 hover:border-emerald-100 dark:hover:border-emerald-900/30 transition-all duration-300 bg-gradient-to-br from-white to-gray-50/80 dark:from-gray-950 dark:to-gray-900/80">
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground mt-1">
              ...
              {data.total.total_requests > 0
                ? `$${(
                    data.total.total_cost / data.total.total_requests
                  ).toFixed(3)}`
                : '-'}
            </div>
          </CardContent>
                  ...
                  <PieChart>
                    ...
                    <Tooltip
                      formatter={(value: any) => [
                        ...
                        `${
                          tokenPieData.find((item) => item.value === value)
                            ?.name === 'openai'
                            ? 'OpenAI'
                            : 'Gemini'
                        }`,
                  ...
                  <PieChart>
                    ...
                    <Tooltip
                      formatter={(value: any) => [
                        `$${value.toFixed(2)}`,
                        `${
                          costPieData.find((item) => item.value === value)
                            ?.name === 'openai'
                            ? 'OpenAI'
                            : 'Gemini'
                        }`,
                ...
                <LineChart data={mergedDailyData}>
                  ...
                  <Tooltip
                    formatter={(value, name) => [
                      ...
                      String(name).includes('cost') ? '費用' : '令牌',
                    ]}
                    labelFormatter={(label) =>
                      format(parseISO(label), 'yyyy-MM-dd', { locale: zhTW })
                    }
                    contentStyle={{
                      backgroundColor: 'rgba(255, 255, 255, 0.9)',
                      borderRadius: '6px',
                ...
                <LineChart data={mergedDailyData}>
                  ...
                  <Tooltip
                    formatter={(value, name) => [
                      ...
                      String(name).includes('openai') ? 'OpenAI' : 'Gemini',
                    ]}
                    labelFormatter={(label) =>
                      format(parseISO(label), 'yyyy-MM-dd', { locale: zhTW })
                    }
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={modelComparisonData}>
                  ...
                  <Tooltip
                    formatter={(value, name) => {
                      if (name === 'tokens')
                        return [value.toLocaleString(), '總令牌數'];
                      if (name === 'cost')
                        return [`$${Number(value).toFixed(2)}`, '總費用'];
                      if (name === 'requests')
                        return [value.toLocaleString(), '請求數'];
                      if (name === 'avg_tokens')
                        return [value.toLocaleString(), '平均每請求令牌數'];
                      return [value, name];
                    }}
                    contentStyle={{
                      backgroundColor: 'rgba(255, 255, 255, 0.9)',
