...
export function ModelsTable({ data }: ModelsTableProps) {
  ...
  return (
    <Card className="border border-gray-100 dark:border-gray-800 hover:border-emerald-100 dark:hover:border-emerald-900/30 transition-all duration-300 bg-gradient-to-br from-white to-gray-50/80 dark:from-gray-950 dark:to-gray-900/80">
      <CardHeader>
        <CardTitle className="text-gray-900 dark:text-gray-100">模型使用詳情</CardTitle>
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="hidden md:table-cell">每千令牌成本</TableHead>
                <TableHead className="hidden md:table-cell">平均每次請求令牌數</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {models.length > 0 ? (
                Object.entries(models).map(([modelName, data]) => {
                  ...
                  return (
                    <TableRow key={modelName} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50">
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: modelColor }}></div>
                          <span>{modelName === 'openai' ? 'OpenAI' : modelName === 'gemini' ? 'Google Gemini' : modelName}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <div className="flex justify-between">
                            ...
                            <span className="text-gray-500 text-xs">{((data.total_tokens / data.total_tokens) * 100).toFixed(1)}%</span>
                          </div>
                          <Progress value={((data.total_tokens / data.total_tokens) * 100)} className="h-2" 
                            style={{ 
                      ...
                      <TableCell className="hidden md:table-cell">
                        ${((data.total_cost / data.total_tokens) * 1000).toFixed(3)}
                      </TableCell>
                      <TableCell className="hidden md:table-cell">
                        {Math.round(data.total_tokens / data.requests).toLocaleString()}
                      </TableCell>
                    </TableRow>
                  );
                })
